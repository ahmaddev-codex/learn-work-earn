# 📅 Work Application Timeline

> Complete timeline for job searching and applying to software engineering roles across UK, Canada, Australia, and remote.

---

## 🗓️ Master Timeline Overview

```
Jul 2026    Aug 2026    Sep 2026    Oct 2026    Nov 2026    Dec 2026
   |            |            |            |            |            |
   [==== PREPARATION ====]  [==== APPLICATIONS BEGIN ====]
                            UK/Canada    Ramp up      Interviews
                            Australia    applications begin

Jan 2027    Feb 2027    Mar 2027    Apr 2027    May 2027    Jun 2027
   |            |            |            |            |            |
   [==== INTERVIEWS & OFFERS ====]        [==== DECISION & ONBOARDING ====]
   Peak       Final        Compare        Negotiate    Relocate     Start!
   interview  offers       offers
   season
```

---

## Phase 1: Preparation (July - September 2026)

### July 2026

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Jul 1-7 | Update LinkedIn profile with professional photo, headline, summary | Jul 7 | 🔴 High | ⬜ |
| Jul 8-14 | Update GitHub profile - pin best projects, update READMEs | Jul 14 | 🔴 High | ⬜ |
| Jul 15-21 | Create/update personal portfolio website | Jul 21 | 🟡 Medium | ⬜ |
| Jul 22-31 | Audit and improve existing projects - add tests, docs, CI/CD | Jul 31 | 🟡 Medium | ⬜ |

**Key Deliverables:**
- [ ] LinkedIn profile optimized (95%+ completeness)
- [ ] GitHub profile with 3-6 pinned projects
- [ ] Portfolio website live (optional but recommended)
- [ ] All projects have good READMEs, tests, and documentation

**LinkedIn Optimization Checklist:**
- [ ] Professional headshot
- [ ] Compelling headline (not just "Software Engineer")
- [ ] Detailed summary with keywords
- [ ] All work experience with bullet points
- [ ] Skills section with 50+ relevant skills
- [ ] Featured section with projects/articles
- [ ] Open to work signal (if comfortable)
- [ ] Custom LinkedIn URL

---

### August 2026

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Aug 1-7 | Start LeetCode practice - 1-2 problems/day | Aug 7 | 🔴 High | ⬜ |
| Aug 8-14 | Build 1 new portfolio project (full-stack or ML) | Aug 14 | 🟡 Medium | ⬜ |
| Aug 15-21 | Study system design basics (Designing Data-Intensive Apps) | Aug 21 | 🔴 High | ⬜ |
| Aug 22-31 | Practice behavioral questions (STAR method) | Aug 31 | 🔴 High | ⬜ |

**LeetCode Study Plan:**

| Week | Focus Area | Problems to Complete | Difficulty |
|------|-----------|---------------------|------------|
| Aug 1-7 | Arrays & Strings | 15 problems | Easy-Medium |
| Aug 8-14 | Hash Tables & Maps | 15 problems | Easy-Medium |
| Aug 15-21 | Two Pointers & Sliding Window | 15 problems | Medium |
| Aug 22-31 | Binary Search & Sorting | 15 problems | Medium |
| Sep 1-7 | BFS & DFS | 15 problems | Medium |
| Sep 8-14 | Dynamic Programming | 15 problems | Medium-Hard |
| Sep 15-21 | Graphs & Trees | 15 problems | Medium |
| Sep 22-30 | System Design & Misc | 10 problems | Medium |

**Total Target: 125 problems by end of September**

**System Design Topics to Cover:**
- [ ] Horizontal vs Vertical Scaling
- [ ] Load Balancing
- [ ] Caching (CDN, Redis, Memcached)
- [ ] Database Scaling (Sharding, Replication, Indexing)
- [ ] Microservices vs Monoliths
- [ ] Message Queues (Kafka, RabbitMQ, SQS)
- [ ] CAP Theorem
- [ ] Consistent Hashing
- [ ] Rate Limiting
- [ ] Design URL Shortener
- [ ] Design Twitter/News Feed
- [ ] Design Uber/Lyft
- [ ] Design Chat Application
- [ ] Design Web Crawler
- [ ] Design Rate Limiter

---

### September 2026

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Sep 1-7 | Draft master resume (2 pages) | Sep 7 | 🔴 High | ⬜ |
| Sep 8-14 | Create tailored resumes for UK, Canada, Australia | Sep 14 | 🔴 High | ⬜ |
| Sep 15-21 | Research 50 target companies (use companies.md) | Sep 21 | 🔴 High | ⬜ |
| Sep 22-30 | Set up job alerts on LinkedIn, Indeed, Glassdoor | Sep 30 | 🟡 Medium | ⬜ |

**Resume Tips by Country:**

**UK Resume:**
- 2 pages maximum
- Include visa status clearly
- Mention right to work
- Use British English spelling
- Include A-levels or equivalent

**Canada Resume:**
- 2 pages maximum
- Include visa/PR status
- Mention if you have Canadian experience
- Use Canadian English spelling
- Include co-op experience if applicable

**Australia Resume:**
- 2-3 pages acceptable
- Include visa status
- Mention if you have Australian experience
- Use Australian English spelling
- Include references available upon request

**Remote/US Resume:**
- 1 page for <5 years experience, 2 pages for 5+
- Focus on impact and metrics
- Use action verbs
- Include GitHub/portfolio links
- Tailor to each job description

---

## Phase 2: Application (October - December 2026)

### October 2026

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Oct 1-7 | Apply to 5 companies (UK focus) | Oct 7 | 🔴 High | ⬜ |
| Oct 8-14 | Apply to 5 companies (Canada focus) | Oct 14 | 🔴 High | ⬜ |
| Oct 15-21 | Apply to 5 companies (Australia focus) | Oct 21 | 🔴 High | ⬜ |
| Oct 22-31 | Apply to 5 companies (Remote focus) | Oct 31 | 🔴 High | ⬜ |

**Application Tracking:**

| Company | Role | Location | Date Applied | Status | Next Step | Notes |
|---------|------|----------|-------------|--------|-----------|-------|
| | | | | | | |
| | | | | | | |
| | | | | | | |
| | | | | | | |
| | | | | | | |

**Weekly Application Goal: 5-10 companies**

---

### November 2026

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Nov 1-7 | Continue applications (10 companies) | Nov 7 | 🔴 High | ⬜ |
| Nov 8-14 | Continue applications (10 companies) | Nov 14 | 🔴 High | ⬜ |
| Nov 15-21 | Follow up on applications sent in Oct | Nov 21 | 🟡 Medium | ⬜ |
| Nov 22-30 | Continue applications (10 companies) | Nov 30 | 🔴 High | ⬜ |

**Networking Activities:**
- [ ] Attend 1 virtual tech meetup per week
- [ ] Connect with 5 recruiters on LinkedIn per week
- [ ] Comment on 3 posts from target companies per week
- [ ] Reach out to 2 employees at target companies per week
- [ ] Post 1 technical article or project update per week

---

### December 2026

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Dec 1-7 | Continue applications (10 companies) | Dec 7 | 🔴 High | ⬜ |
| Dec 8-14 | Follow up on all pending applications | Dec 14 | 🟡 Medium | ⬜ |
| Dec 15-21 | Prepare for technical interviews | Dec 21 | 🔴 High | ⬜ |
| Dec 22-31 | Holiday break + light interview prep | Dec 31 | 🟢 Low | ⬜ |

**Interview Preparation Checklist:**
- [ ] LeetCode: 150+ problems solved
- [ ] System design: 5+ mock designs completed
- [ ] Behavioral: 10+ STAR stories prepared
- [ ] Company research: 2-3 facts per company
- [ ] Questions ready: 3-5 per interview
- [ ] Technical deep dive: Know your projects inside out

---

## Phase 3: Interviews & Offers (January - March 2027)

### January 2027

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Jan 1-7 | Attend recruiter screens (expect 3-5) | Jan 7 | 🔴 High | ⬜ |
| Jan 8-14 | Attend technical screens (expect 2-4) | Jan 14 | 🔴 High | ⬜ |
| Jan 15-21 | Attend on-site/virtual interviews (expect 1-3) | Jan 21 | 🔴 High | ⬜ |
| Jan 22-31 | Continue applications + interview prep | Jan 31 | 🔴 High | ⬜ |

**Interview Week Schedule Example:**

| Day | Time | Activity | Company |
|-----|------|----------|---------|
| Monday | 10:00 AM | Recruiter Screen | Company A |
| Monday | 2:00 PM | LeetCode practice | - |
| Tuesday | 11:00 AM | Technical Phone Screen | Company B |
| Tuesday | 4:00 PM | System design study | - |
| Wednesday | 10:00 AM | Recruiter Screen | Company C |
| Wednesday | 2:00 PM | Behavioral prep | - |
| Thursday | 9:00 AM | Virtual On-site (4 hrs) | Company D |
| Thursday | 3:00 PM | Rest + follow-up emails | - |
| Friday | 10:00 AM | Recruiter Screen | Company E |
| Friday | 2:00 PM | LeetCode + project work | - |
| Weekend | - | Rest, light prep | - |

---

### February 2027

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Feb 1-7 | Attend final round interviews | Feb 7 | 🔴 High | ⬜ |
| Feb 8-14 | Receive first offers | Feb 14 | 🔴 High | ⬜ |
| Feb 15-21 | Compare offers, research companies | Feb 21 | 🔴 High | ⬜ |
| Feb 22-28 | Begin negotiations | Feb 28 | 🔴 High | ⬜ |

**Offer Evaluation Matrix:**

| Factor | Weight | Offer A | Offer B | Offer C |
|--------|--------|---------|---------|---------|
| **Base Salary** | 20% | ___ | ___ | ___ |
| **Equity/Bonus** | 15% | ___ | ___ | ___ |
| **Growth Opportunity** | 15% | ___ | ___ | ___ |
| **Tech Stack** | 10% | ___ | ___ | ___ |
| **Company Culture** | 10% | ___ | ___ | ___ |
| **Location/Remote** | 10% | ___ | ___ | ___ |
| **Visa Sponsorship** | 10% | ___ | ___ | ___ |
| **Work-Life Balance** | 10% | ___ | ___ | ___ |
| **Total Score** | 100% | ___ | ___ | ___ |

---

### March 2027

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Mar 1-7 | Final negotiations | Mar 7 | 🔴 High | ⬜ |
| Mar 8-14 | Accept best offer | Mar 14 | 🔴 High | ⬜ |
| Mar 15-21 | Decline other offers gracefully | Mar 21 | 🟡 Medium | ⬜ |
| Mar 22-31 | Prepare for start date | Mar 31 | 🟡 Medium | ⬜ |

---

## Phase 4: Decision & Onboarding (April - June 2027)

### April 2027

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Apr 1-7 | Apply for work visa (if needed) | Apr 7 | 🔴 High | ⬜ |
| Apr 8-14 | Research housing in target city | Apr 14 | 🟡 Medium | ⬜ |
| Apr 15-21 | Book temporary accommodation | Apr 21 | 🟡 Medium | ⬜ |
| Apr 22-30 | Plan relocation logistics | Apr 30 | 🟡 Medium | ⬜ |

**Visa Application Timeline:**

| Country | Visa Type | Apply By | Processing Time |
|---------|-----------|----------|-----------------|
| UK | Skilled Worker | Apr 2027 | 3 weeks |
| UK | Graduate Visa | Apr 2027 | 3 weeks |
| Canada | PGWP | Apr 2027 | 4-12 weeks |
| Canada | Express Entry | Apr 2027 | 6-8 months |
| Australia | 485 Visa | Apr 2027 | 4-8 weeks |
| Australia | TSS (482) | Apr 2027 | 6-10 weeks |

---

### May 2027

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| May 1-7 | Book flights | May 7 | 🔴 High | ⬜ |
| May 8-14 | Arrange shipping/moving | May 14 | 🟡 Medium | ⬜ |
| May 15-21 | Set up bank account (if possible remotely) | May 21 | 🟡 Medium | ⬜ |
| May 22-31 | Final preparations, say goodbye | May 31 | 🟢 Low | ⬜ |

---

### June 2027

| Week | Task | Deadline | Priority | Status |
|------|------|----------|----------|--------|
| Jun 1-7 | Travel to destination | Jun 7 | 🔴 High | ⬜ |
| Jun 8-14 | Settle in, find permanent housing | Jun 14 | 🔴 High | ⬜ |
| Jun 15-21 | Explore city, set up utilities | Jun 21 | 🟡 Medium | ⬜ |
| Jun 22-30 | Start new job! 🎉 | Jun 30 | 🎉 | ⬜ |

---

## 📋 Monthly Checklist Summary

### July 2026
- [ ] Update LinkedIn
- [ ] Update GitHub
- [ ] Create portfolio website
- [ ] Improve existing projects

### August 2026
- [ ] LeetCode 1-2 problems/day
- [ ] Build new portfolio project
- [ ] Study system design
- [ ] Practice behavioral questions

### September 2026
- [ ] Draft master resume
- [ ] Create country-specific resumes
- [ ] Research 50 target companies
- [ ] Set up job alerts

### October 2026
- [ ] Apply to 20 companies (5 per region)
- [ ] Track all applications
- [ ] Customize cover letters
- [ ] Follow up on applications

### November 2026
- [ ] Apply to 30 more companies
- [ ] Network on LinkedIn
- [ ] Attend virtual meetups
- [ ] Follow up on October applications

### December 2026
- [ ] Apply to 20 more companies
- [ ] Follow up on all pending applications
- [ ] Intensify interview prep
- [ ] Holiday break

### January 2027
- [ ] Attend 3-5 recruiter screens
- [ ] Attend 2-4 technical screens
- [ ] Attend 1-3 on-site interviews
- [ ] Continue applications

### February 2027
- [ ] Attend final round interviews
- [ ] Receive first offers
- [ ] Compare offers
- [ ] Begin negotiations

### March 2027
- [ ] Final negotiations
- [ ] Accept best offer
- [ ] Decline other offers
- [ ] Prepare for start date

### April 2027
- [ ] Apply for work visa
- [ ] Research housing
- [ ] Book temporary accommodation
- [ ] Plan relocation

### May 2027
- [ ] Book flights
- [ ] Arrange moving
- [ ] Set up bank account
- [ ] Final preparations

### June 2027
- [ ] Travel
- [ ] Settle in
- [ ] Find housing
- [ ] Start new job! 🎉

---

## 🔔 Important Reminders

### Set Calendar Reminders For:
- [ ] **2026-07-01**: Start LinkedIn/GitHub updates
- [ ] **2026-08-01**: Start LeetCode daily practice
- [ ] **2026-09-01**: Draft resumes and research companies
- [ ] **2026-10-01**: Start applications (UK, Canada, Australia)
- [ ] **2026-11-01**: Ramp up applications — target 30 total by month end
- [ ] **2026-12-01**: Intensify interview prep + most Canada/UK deadlines
- [ ] **2027-01-01**: Interview season begins — expect 3-5 recruiter screens
- [ ] **2027-02-01**: Expect first offers
- [ ] **2027-03-15**: Decision deadline — finalize choice
- [ ] **2027-04-01**: Apply for work visa or study permit
- [ ] **2027-05-01**: Book flights + begin relocation arrangements
- [ ] **2027-06-01**: Relocate and start!

---

## 🎯 Application Metrics to Track

| Metric | Target | Actual |
|--------|--------|--------|
| **Total Applications Sent** | 100+ | ___ |
| **Recruiter Screens** | 15-25 | ___ |
| **Technical Screens** | 8-15 | ___ |
| **On-site/Virtual Interviews** | 5-10 | ___ |
| **Final Round Interviews** | 3-5 | ___ |
| **Offers Received** | 2-4 | ___ |
| **Offer Acceptance Rate** | 100% | ___ |
| **Response Rate** | 20-30% | ___ |

---

---

## 🗺️ Status Legend
| ⬜ Not Started | 🔵 In Progress | ✅ Done | ❌ Skipped |

---

*Last updated: 2026-06-29 | Adjust all dates based on your specific programs and target countries.*
